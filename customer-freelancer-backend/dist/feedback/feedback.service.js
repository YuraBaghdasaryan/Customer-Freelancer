...
let FeedbackService = class FeedbackService {
    ...
    async create(createFeedbackDto) {
        ...
        const { user, jobId } = createFeedbackDto, data = __rest(createFeedbackDto, ["user", "jobId"]);
        console.log(user);
        if (job.customerId == user.customer.id && job.status == 2) {
            await this.jobRepository.update({ id: jobId }, data);
            return 'adds a new feedback';
        }
        else {
            if (job.status != 2) {
                throw new common_1.NotFoundException('Oops! that job is not done');
            }
            else {
